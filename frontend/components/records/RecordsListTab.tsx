...
export const RecordsListTab: React.FC<RecordsListTabProps> = ({
  ...
  const handleShareNotice = (title: string, description: string) => {
    openNoticeModal(title, description);
    setActiveMenuId(null);
  };
                    ...
                    <div className="absolute right-0 top-full mt-1.5 backdrop-blur-xl rounded-[1.25rem] shadow-xl p-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-200 min-w-[160px]" style={{ background: palette.cardBgStrong, border: `1px solid ${palette.border}` }}>
                      <div className="flex flex-col gap-0.5">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleShareNotice('준비 중인 공유예요', '카카오톡 공유 기능은 아직 연결 전이에요.');
                          }}
                          className="flex items-center gap-2.5 px-3 py-2 rounded-xl w-full text-left transition-colors group/btn"
                        >
                          <div className="rounded-md p-1 transition-colors" style={{ background: palette.cardBgSoft }}>
                            <MessageCircle size={14} className="text-mist-500" />
                          </div>
                          <span className="text-[11px] font-bold" style={{ color: palette.strongText }}>카카오톡 공유</span>
                        </button>

                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleShareNotice('준비 중인 공유예요', '링크 복사 기능은 아직 연결 전이에요.');
                          }}
                          className="flex items-center gap-2.5 px-3 py-2 rounded-xl w-full text-left transition-colors group/btn"
                        >
                          <div className="rounded-md p-1 transition-colors" style={{ background: palette.cardBgSoft }}>
                            <Link2 size={14} className="text-mist-500" />
                          </div>
                          <span className="text-[11px] font-bold" style={{ color: palette.strongText }}>링크 복사</span>
                        </button>
